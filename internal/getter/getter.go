...
func SyncGetBlock(p context.Context, k cid.Cid, gb GetBlocksFunc) (blocks.Block, error) {
	...
	fmt.Printf("[%s] BITSWAP GETTER", k.String())
...
func handleIncoming(ctx context.Context, sessctx context.Context, remaining *cid.Set,
	...
	// ça marche pas fmt.Println("[%s] FOUND VIA BITSWAP 42\n", blk.Cid().String())
	...
	for {
		...
		case blk, ok := <-in:
			...
			fmt.Println("[%s] FOUND VIA BITSWAP from getter handleIncoming", blk.Cid().String())
